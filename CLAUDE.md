...
- `scripts/main push` — production build + htmlproofer + `aws sso login` + real `rclone sync` (with `-v --log-file=tmp/rclone.log`) to `s3-for-carterpape-com:carterpape.com`, then Cloudflare cache purge for just the synced paths, then `git pull && git push`. git-lfs uses a self-hosted Cloudflare Worker (`~/Developer/git-lfs-self-host/`); credentials are cached in the macOS Keychain for the Worker host.
